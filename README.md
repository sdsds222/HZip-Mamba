...
1.  **语义转折点定位与方向判断（新增起点/终点概念）：**
    * **现象：** 遇到转折词（如“但是”）或主题切换时，G f 和 $G_b$ **同时**或**几乎同时**剧烈增大，标记为**语义变化起点**。
    * **作用：** 激活门控 $g_f, g_b$。门控网络通过比较  G f 和 $G_b$ 的相对大小和趋势，**推断语义变化的方向**（例如：正向信息流是主要的推动力）。实现**语义主题的即时切换**。
...
3.  **背景信息过滤与噪音排除：**
    ...
    * **作用：** 门控 $g_f, g_b$ 保持中性或抑制。当 G f 高而 $G_b$ 低时，模型可推断其为**局部噪音**或**孤立变化**，实现对**非关键信息的过滤**和排除。
